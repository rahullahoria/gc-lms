...
export default function Quiz() {
  ...
  const [questionResponsesFake, setQuestionResponsesFake] = useState([]);
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        ...
        setQuestionResponsesFake(new Array(responseData.questions.length).fill(0));
  ...
  const isMobileMatch = window.innerWidth < 600;
  return (
    <>
      {!isLoading && loadedQuiz && (
        <>
          <Stack
            spacing={isMobileMatch ? 1 : 50}
            justifyContent={isMobileMatch ? "flex-start" : "space-between"}
            direction={isMobileMatch ? "column" : "row"}
          >
            <Typography variant="h6" component="h2">
              {loadedQuiz.name} ({questionIndex + 1}/
              ...
              <Card sx={{ maxWidth: 800, margin: "auto", marginTop: "25px" }}>
                <CardContent>
                  <Typography gutterBottom variant="h5" component="div">
                    {questionIndex + 1}.{" "}
                    {loadedQuiz.questions[questionIndex].Question}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {!updatingResponse && (
                      <FormControl>
                        <RadioGroup
                          ...
                          value={
                            questionResponses[
                              loadedQuiz.questions[questionIndex].qIndex
                            ] || false
                          }
                    ...
                    />))} */}
                          ...
                          {loadedQuiz.questions[questionIndex].optionsOrder.map(
                            (value) => (
                              <FormControlLabel
                                value={value}
                                control={<Radio />}
                                label={
                                  loadedQuiz.questions[questionIndex][value]
                                }
                              />
                            )
                          )}
                ...
                <CardContent>
                  ...
                  >
                    ...
                    {questionIndex < loadedQuiz.questions.length - 1 && (
                      <Button
                        size="small"
                        onClick={() => {
                          setQuestionIndex(questionIndex + 1);
                ...
                >
                  <Box sx={style}>
                    ...
                    <Button variant="contained" onClick={submitQuestionResponse}>Yes</Button>
              ...
              <Card sx={{ maxWidth: 800, margin: "auto", marginTop: "25px" }}>
                <CardContent>
                  <Stack
                    ...
                    direction="row"
                  >
                    {questionResponsesFake.map((res, index) => (
                      <IconButton
                        style={{ width: "30px" }}
                        size="small"
